...
public class Flying : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ViveInput.GetPress(HandRole.LeftHand, ControllerButton.FullTrigger)) flying();
    }

    private void flying()
    {
        float rightHandTurnX = leftHand.transform.eulerAngles.x;
        float rightHandTurnY = leftHand.transform.eulerAngles.y;
    ...
    {
        Vector3 direction = (leftHand.transform.position - head.transform.position) / speed;
        transform.position += direction;
    }
}
